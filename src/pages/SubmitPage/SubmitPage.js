...
const SubmitPage = () => {
  ...
  const date = data?.vaccinated_at?.toLocaleDateString('en-US', { year: 'numeric', month: 'numeric', day: 'numeric' }).split('/')

  const covid = data?.had_covid_at?.toLocaleDateString('en-US', { year: 'numeric', month: 'numeric', day: 'numeric' }).split('/')
  let sendData = {
    ...
    ...(data.had_covid_at && { "had_covid_at": `${covid[2]}-${covid[0]}-${covid[1]}` }),
    "vaccinated": data.vaccinated === 'false' ? false : true,
    ...(data.vaccinated_at && { "vaccinated_at": `${date[2]}-${date[0]}-${date[1]}` }),
