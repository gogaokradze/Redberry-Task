...
const Insights = () => {
  ...
  const insights = submitedData => {
    setData({
      ...data,
      will_organize_devtalk: submitedData.will_organize_devtalk,
      devtalk_topic: submitedData.devtalk_topic,
      something_special: submitedData.something_special,
    })
  ...
  useEffect(() => {
    return () => {
      setData({
        ...data,
        will_organize_devtalk: getValues().will_organize_devtalk,
      ...
      <form onSubmit={handleSubmit(insights)} className={classes.form}>
        <div className={classes.formsize}>
          ...
          {Object.keys(errors).length > 0 && (
            <div className={classes.div}>
              <p className={classes.errorMessage}>`*please fill out the form</p>
            </div>
          )}
        </div>
        <FormButton />
